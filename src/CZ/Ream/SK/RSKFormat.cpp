...
public:
    RSKFromDRMMap() noexcept
    {
        ...
        emplace(DRM_FORMAT_XRGB8888,      kRGB_888x_SkColorType);
        emplace(DRM_FORMAT_ABGR8888,      kRGBA_8888_SkColorType);
        emplace(DRM_FORMAT_XBGR8888,      kRGB_888x_SkColorType);
        emplace(DRM_FORMAT_BGRA8888,      kBGRA_8888_SkColorType); // optional alias
        emplace(DRM_FORMAT_RGB888,        kRGB_888x_SkColorType);  // closest match
        emplace(DRM_FORMAT_BGR888,        kRGB_888x_SkColorType);  // closest match

        emplace(DRM_FORMAT_RGB565,        kRGB_565_SkColorType);
        emplace(DRM_FORMAT_ARGB4444,      kARGB_4444_SkColorType);
        emplace(DRM_FORMAT_RGBA4444,      kARGB_4444_SkColorType); // may match if swizzled
        ...
        emplace(DRM_FORMAT_R16,           kA16_unorm_SkColorType); // or kR16_unorm if supported
        emplace(DRM_FORMAT_GR1616,        kR16G16_unorm_SkColorType);

        //emplace(DRM_FORMAT_XBGR8888,      kRGB_888x_SkColorType);
        //emplace(DRM_FORMAT_ARGB1555,      kARGB_4444_SkColorType); // closest, non-ideal

        // Optional for sRGB textures
        //emplace(DRM_FORMAT_ABGR8888,      kRGBA_8888_SkColorType); // for sRGB content
    }
};
...
public:
    ...
    {
        //emplace(kAlpha_8_SkColorType,             DRM_FORMAT_A8);
        emplace(kRGB_565_SkColorType,             DRM_FORMAT_RGB565);
        emplace(kARGB_4444_SkColorType,           DRM_FORMAT_ARGB4444);
        emplace(kRGBA_8888_SkColorType,           DRM_FORMAT_ABGR8888); // RGBA → ABGR
        emplace(kRGB_888x_SkColorType,            DRM_FORMAT_XBGR8888); // RGBX → XBGR
        emplace(kBGRA_8888_SkColorType,           DRM_FORMAT_ARGB8888); // BGRA → ARGB
        ...
        emplace(kGray_8_SkColorType,              DRM_FORMAT_R8);  // grayscale → red only
        ...
        // No standard DRM format for RGBA_F32; you’d need a custom or extension format
        emplace(kR8G8_unorm_SkColorType,          DRM_FORMAT_GR88);
        emplace(kA16_float_SkColorType,           DRM_FORMAT_INVALID); // No DRM match
        //emplace(kR16G16_float_SkColorType,        DRM_FORMAT_GR1616F);
        //emplace(kA16_unorm_SkColorType,           DRM_FORMAT_A16);
        emplace(kR16G16_unorm_SkColorType,        DRM_FORMAT_GR1616);
        emplace(kR16G16B16A16_unorm_SkColorType,  DRM_FORMAT_ABGR16161616);
        emplace(kSRGBA_8888_SkColorType,          DRM_FORMAT_ABGR8888); // assume same bits, sRGB via metadata
